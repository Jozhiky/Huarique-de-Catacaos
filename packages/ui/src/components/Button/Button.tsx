...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'brand' | 'gold' | 'coastal' | 'success' | 'danger' | 'outline' | 'ghost';
  size?: 'md' | 'touch' | 'lg';
  fullWidth?: boolean;
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const sizeStyles = {
    ...
    touch: 'min-h-[48px] min-w-[48px] px-5 py-3 text-lg', // Target táctil ergonómico >= 48px
  ...
  const variantStyles = {
    ...
    gold:
      'bg-brand-gold text-brand-navy hover:bg-[#b5954e] active:bg-[#a68641] focus:ring-brand-navy shadow-sm font-extrabold',
    coastal:
